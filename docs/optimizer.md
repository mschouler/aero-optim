...
### Optimizer
The [`Optimizer`](https://github.com/mschouler/aero-optim/blob/add-docs/src/ins_optimizer.py#L35-L161) abstract class extracts general arguments from the `"optim"` and `"study"` dictionaries of the configuration file such as:
...
!!! Note
    All optimizer parameters are described in their respective class definition (see [`Optimizer`](https://github.com/mschouler/aero-optim/blob/master/src/ins_optimizer.py#L40-L72), [`WolfOptimizer`](https://github.com/mschouler/aero-optim/blob/master/src/ins_optimizer.py#L169-L186)).

### Wolf Optimizer
The [`WolfOptimizer`](https://github.com/mschouler/aero-optim/blob/master/src/ins_optimizer.py#L164-L349) class illustrates how `Optimizer` can be inherited to perform a `Wolf`-based optimization.
...
!!! Note
    The [`constraint`](https://github.com/mschouler/aero-optim/blob/master/src/ins_optimizer.py#L200-L212) method based on [`inspyred` documentation](https://inspyred.readthedocs.io/en/latest/recipes.html#constraint-selection) is put in place in order to penalize inadequate geometries both in terms of area and lift coefficient.
