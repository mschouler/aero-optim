...
from aero_optim.optim.optimizer import DebugOptimizer
from aero_optim.optim.optimizer import WolfOptimizer, MusicaaOptimizer
...
class PymooMusicaaOptimizer(MusicaaOptimizer, Problem):
    """
    This class implements a Wolf based Optimizer.
    """
    def __init__(self, config: dict):
        ...
        **Input**
        ...
        - config (dict): the config file dictionary.
        """
        MusicaaOptimizer.__init__(self, config)
        Problem.__init__(
            self, n_var=self.n_design, n_obj=1, n_ieq_constr=2, xl=self.bound[0], xu=self.bound[1]
        )
